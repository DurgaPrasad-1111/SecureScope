...
PASSIVE_PROGRESS = {
    ...
    "http_probe": 42,
    "headers_analysis": 52,
    "technology_fingerprint": 62,
    "url_discovery": 70,
    "osint_public_data": 80,
    "search_engine_recon": 86,
    "vulnerability_surface": 94,
    "risk_scoring": 98,
    "report_generation": 100,
}
ACTIVE_PROGRESS = {
    "port_scan": 15,
    "service_enumeration": 25,
    "http_probe": 35,
    "ssl_check": 42,
    "headers_analysis": 50,
    "technology_fingerprint": 58,
    "url_discovery": 64,
    "directory_bruteforce": 72,
    "auth_testing": 78,
    "session_management": 82,
    "api_endpoint_testing": 86,
    "sql_injection_test": 90,
    "xss_test": 92,
    "vulnerability_scan": 95,
    "vulnerability_surface": 97,
    "risk_scoring": 99,
    "report_generation": 100,
}
FULL_PROGRESS = {
    ...
    "http_probe": 32,
    "headers_analysis": 38,
    "technology_fingerprint": 44,
    "url_discovery": 50,
    "osint_public_data": 56,
    "search_engine_recon": 60,
    "port_scan": 68,
    "service_enumeration": 72,
    "directory_bruteforce": 78,
    "auth_testing": 82,
    "session_management": 85,
    "api_endpoint_testing": 88,
    "sql_injection_test": 91,
    "xss_test": 93,
    "vulnerability_scan": 95,
    "hosting_detection": 96,
    "vulnerability_surface": 97,
    "risk_scoring": 99,
    "report_generation": 100,
}
...
MODULE_STAGE = {
    ...
    "whois": "whois_lookup",
    "ssl_check": "tls_analysis",
    "http_probe": "http_probing",
    "headers_analysis": "header_analysis",
    "technology_fingerprint": "technology_detection",
    "url_discovery": "url_discovery",
    "osint_public_data": "osint_collection",
    ...
    "api_endpoint_testing": "api_endpoint_testing",
    "sql_injection_test": "sql_injection_testing",
    "xss_test": "xss_testing",
    "vulnerability_scan": "cve_scanning",
    "hosting_detection": "hosting_detection",
...
def _truncate_lines(lines: list[str], *, max_lines: int, max_len: int) -> tuple[list[str], bool]:
    truncated = False
    output: list[str] = []
...
def _run_command(
    ...
    *,
    ...
    try:
        ...
        stdout_trimmed, stdout_truncated = _truncate_lines(
            stdout_lines, max_lines=max_stdout_lines, max_len=MAX_LOG_LINE_LENGTH
        )
        stderr_trimmed, stderr_truncated = _truncate_lines(
            stderr_lines, max_lines=max_stderr_lines, max_len=MAX_LOG_LINE_LENGTH
        )
        return {
            "cmd": cmd,
            ...
            "timed_out": False,
            "stdout_lines": stdout_trimmed,
            "stdout_truncated": stdout_truncated,
            "stderr_lines": stderr_trimmed,
...
def _remaining_budget(context: dict) -> float | None:
...
def _cap_timeout(context: dict, requested_seconds: float) -> int:
    remaining = _remaining_budget(context)
    if remaining is None:
        return max(1, int(requested_seconds))
    return max(1, int(min(requested_seconds, remaining)))


def _safe_snippet(value: str | None, max_len: int = 400) -> str:
    if not value:
        return ""
...
def _base_url_from_context(context: dict) -> str | None:
    probe = _module_data(context, "http_probe")
    if isinstance(probe, dict) and probe.get("url"):
        return str(probe.get("url"))
    ...
    *,
    ...
    try:
        response = requests.get(
            url,
            timeout=(max(1.0, min(3.0, timeout_seconds)), max(1.0, timeout_seconds)),
            allow_redirects=allow_redirects,
            headers={"User-Agent": "ReconEngine/1.0"},
            stream=True,
        )
        snippet_bytes = b""
        for chunk in response.iter_content(chunk_size=1024):
            if not chunk:
                continue
            snippet_bytes += chunk
            if len(snippet_bytes) >= max_bytes:
                break
        return {
            "url": url,
...
def _doh_resolve_record(name: str, record_type: str, timeout: float = 3.0) -> list[str]:
    try:
        response = requests.get(
            ...
            timeout=(timeout, max(1.0, timeout)),
...
def _http_probe(target: str, *, allow_private_targets: bool = False) -> dict:
    ...
    for scheme in ("https", "http"):
        ...
        try:
            response = requests.get(
                url,
                timeout=(4, 6),
                allow_redirects=bool(allow_private_targets),
                stream=True,
            ...
            if not allow_private_targets and response.is_redirect and response_headers.get("Location"):
                redirects.append(response_headers.get("Location"))
            return {
                "url": response.url,
                "status_code": response.status_code,
...
def _rdap_lookup(target: str, *, timeout_seconds: float = 8.0) -> dict:
    try:
        ipaddress.ip_address(target)
    ...
    connect_timeout = max(1.0, min(3.0, timeout_seconds))
    read_timeout = max(connect_timeout, timeout_seconds)
    response = requests.get(
        rdap_url,
        timeout=(connect_timeout, read_timeout),
        headers={"Accept": "application/json", "User-Agent": "ReconEngine/1.0"},
    )
...
def _query_whois_server(
    server: str, query: str, *, max_bytes: int = MAX_WHOIS_BYTES, timeout_seconds: float = 10.0
) -> str:
...
def _socket_whois_lookup(target: str, *, timeout_seconds: float = 10.0) -> dict:
    bootstrap = _query_whois_server("whois.iana.org", target, timeout_seconds=timeout_seconds)
    referred_server = _extract_whois_referral(bootstrap) or "whois.iana.org"
    detailed = _query_whois_server(referred_server, target, timeout_seconds=timeout_seconds)
...
def _whois_lookup(target: str, *, timeout_seconds: float = 10.0) -> dict:
    try:
        return _rdap_lookup(target, timeout_seconds=timeout_seconds)
    except Exception as rdap_error:
        try:
            socket_whois_result = _socket_whois_lookup(target, timeout_seconds=timeout_seconds)
...
def _subdomain_enumeration(target: str, *, context: dict | None = None) -> dict:
    ...
    tool_timeout = _cap_timeout(ctx, float(ctx.get("subdomain_tool_timeout_seconds", 30)))
    cli_timeout = max(1, min(10, tool_timeout))

    try:
        if _tool_available("subfinder"):
            subfinder = _run_command(
                ["subfinder", "-silent", "-duc", "-timeout", str(cli_timeout), "-max-time", "1", "-d", domain],
        ...
        raw["tools"]["subfinder"] = {"error": str(e), "status": "failed"}

    try:
        if _tool_available("assetfinder"):
            assetfinder = _run_command(["assetfinder", "--subs-only", domain], timeout_seconds=tool_timeout)
        ...
        raw["tools"]["assetfinder"] = {"error": str(e), "status": "failed"}

    try:
        if _tool_available("amass"):
            amass = _run_command(["amass", "enum", "-silent", "-timeout", "1", "-d", domain], timeout_seconds=tool_timeout)
        ...
        raw["tools"]["amass"] = {"error": str(e), "status": "failed"}
    ...
    def resolve_candidate(candidate: str) -> tuple[str, list[str]]:
        try:
            ...
            logger.warning(f"Failed to resolve {candidate}: {e}")
            resolved_ips = []
...
def _module_whois(context: dict) -> dict:
    whois_timeout = _cap_timeout(context, float(context.get("whois_timeout_seconds", 10)))
    return _whois_lookup(context["target"], timeout_seconds=whois_timeout)
...
def _module_port_scan(context: dict) -> dict:
    ...
    raw: dict[str, Any] = {"tools": {}}
    port_timeout = _cap_timeout(context, float(context.get("port_scan_timeout_seconds", 180)))
    ...
    if _tool_available("naabu"):
        ...
        for cmd in attempts:
            result = _run_command(cmd, timeout_seconds=port_timeout)
...
def _module_http_probe(context: dict) -> dict:
    return _http_probe(
        context["target"],
        allow_private_targets=bool(context.get("allow_private_targets")),
    )
...
def _module_url_discovery(context: dict) -> dict:
    ...
    raw: dict[str, Any] = {"tools": {}}
    urls: set[str] = set()
    url_timeout = _cap_timeout(context, float(context.get("url_discovery_timeout_seconds", 90)))
    gau_timeout = max(5, min(20, url_timeout))

    if _tool_available("gau"):
        gau = _run_command(
            ["gau", "--subs", "--threads", "4", "--timeout", str(gau_timeout), target],
    ...
    if _tool_available("waybackurls"):
        wayback = _run_command(["waybackurls"], timeout_seconds=url_timeout, stdin_text=f"{target}\n")
...
def _extract_meta_from_html(html: str) -> dict[str, str]:
    if not html:
        return {}
    meta: dict[str, str] = {}
    patterns = {
        "generator": r'name=["\']generator["\'][^>]*content=["\']([^"\']+)["\']',
        "application_name": r'name=["\']application-name["\'][^>]*content=["\']([^"\']+)["\']',
        "author": r'name=["\']author["\'][^>]*content=["\']([^"\']+)["\']',
    }
    for key, pattern in patterns.items():
        match = re.search(pattern, html, flags=re.IGNORECASE)
        if match:
            meta[key] = _safe_snippet(match.group(1), 200)
    title_match = re.search(r"<title[^>]*>(.*?)</title>", html, flags=re.IGNORECASE | re.DOTALL)
    if title_match:
        meta["title"] = _safe_snippet(re.sub(r"\s+", " ", title_match.group(1)).strip(), 200)
    return meta


def _module_osint_public_data(context: dict) -> dict:
    base_url = _base_url_from_context(context)
    if not base_url:
        return {"public_files": [], "metadata": {}, "notes": ["base_url_unavailable"]}

    timeout_seconds = _cap_timeout(context, 3.0)
    paths = [
        "/robots.txt",
        "/sitemap.xml",
    ...
    for path in paths:
        ...
        resp = _fetch_url(url, timeout_seconds=timeout_seconds, max_bytes=6000)
            ...
            public_files.append(
                {
                    ...
                    "snippet": _safe_snippet(resp.get("snippet")),
                }
            )

    probe_data = _module_data(context, "http_probe")
    html_snippet = str(probe_data.get("html_snippet") or "") if isinstance(probe_data, dict) else ""
    headers = probe_data.get("headers") if isinstance(probe_data, dict) else {}
    if not isinstance(headers, dict):
        headers = {}

    metadata = _extract_meta_from_html(html_snippet)
    powered_by = headers.get("X-Powered-By") or headers.get("x-powered-by")
    if powered_by:
        metadata["powered_by"] = _safe_snippet(str(powered_by), 200)

    return {
        "public_files": public_files,
        "metadata": metadata,
        "github_leaks": [],
        "notes": ["github_leaks_disabled_without_token"],
    }


def _module_search_engine_recon(context: dict) -> dict:
    target = str(context.get("target") or "").strip()
    if not target or _is_ip_target(target):
        return {"engine": "duckduckgo", "query": "", "results": [], "notes": ["invalid_target"]}

    query = f"site:{target}"
    timeout_seconds = _cap_timeout(context, 4.0)
    results: list[str] = []
    error = None
    try:
        response = requests.get(
        ...
        if response.ok:
            ...
            error = f"search_failed_http_{response.status_code}"
    except Exception as exc:
        error = str(exc)
    ...
    return {
        "engine": "duckduckgo",
        "query": query,
        "results": sorted(set(results))[:10],
        "suggested_dorks": suggested_dorks,
        "error": error,
    }


def _module_service_enumeration(context: dict) -> dict:
    ...
    for port in open_ports or []:
        try:
            ...
            continue
        services.append({"port": port_int, "service": SERVICE_MAP.get(port_int, "unknown")})
    return {"services": services, "count": len(services)}


def _module_directory_bruteforce(context: dict) -> dict:
    base_url = _base_url_from_context(context)
    if not base_url:
        return {"tested": [], "found": [], "responses": [], "notes": ["base_url_unavailable"]}

    timeout_seconds = _cap_timeout(context, 2.0)
    paths = [
        "/admin",
        "/login",
        "/signin",
    ...
    for path in paths:
        ...
        if isinstance(status, int) and status in {200, 301, 302, 401, 403}:
            found.append(response_entry)

    sensitive_files = [entry for entry in found if entry.get("path") in {"/.env", "/.git/config"} and entry.get("status") == 200]
    return {
        "tested": paths,
        "found": found,
        "responses": responses,
...
def _module_auth_testing(context: dict) -> dict:
    base_url = _base_url_from_context(context) or ""
    dir_data = _module_data(context, "directory_bruteforce")
    found = dir_data.get("found") if isinstance(dir_data, dict) else []
    login_endpoints: list[dict[str, Any]] = []
    for entry in found or []:
        path = str(entry.get("path") or "")
        if any(token in path for token in ("/login", "/signin", "/auth")):
            login_endpoints.append(entry)

    issues: list[dict[str, Any]] = []
    if login_endpoints and base_url.startswith("http://"):
        issues.append(
            {
                "title": "Login page accessible over HTTP",
                "severity": "high",
                "raw_output": f"Login endpoint served over HTTP at {login_endpoints[0].get('url')}",
                "mitigation": "Enforce HTTPS for all authentication endpoints and enable HSTS.",
            }
        )

    for endpoint in login_endpoints:
        snippet = str(endpoint.get("snippet") or "").lower()
        if "type=\"password\"" in snippet and "method=\"get\"" in snippet:
            issues.append(
                {
                    "title": "Login form uses GET method",
                    "severity": "medium",
                    "raw_output": _safe_snippet(endpoint.get("snippet")),
                    "mitigation": "Ensure login forms submit via POST and include CSRF protection.",
                }
            )

    return {"login_endpoints": login_endpoints, "issues": issues}


def _module_session_management(context: dict) -> dict:
    probe = _module_data(context, "http_probe")
    headers = probe.get("headers") if isinstance(probe, dict) else {}
    if not isinstance(headers, dict):
        headers = {}
            ...
            issues.append(
                {
                    ...
                    "severity": "medium",
                    "raw_output": _safe_snippet(str(set_cookie), 300),
                    "mitigation": "Set the Secure flag on all session cookies to prevent downgrade over HTTP.",
                }
            )
            ...
            issues.append(
                {
                    ...
                    "severity": "medium",
                    "raw_output": _safe_snippet(str(set_cookie), 300),
                    "mitigation": "Enable HttpOnly on session cookies to reduce XSS session theft risk.",
                }
            )
        if "samesite" not in cookie_lower:
            issues.append(
                {
                    "title": "Session cookies missing SameSite policy",
                    "severity": "low",
                    "raw_output": _safe_snippet(str(set_cookie), 300),
                    "mitigation": "Set SameSite=Lax or SameSite=Strict on session cookies to mitigate CSRF.",
                }
            )
    return {"set_cookie": _safe_snippet(str(set_cookie), 500), "issues": issues}


def _module_api_endpoint_testing(context: dict) -> dict:
    base_url = _base_url_from_context(context)
    if not base_url:
        return {"tested": [], "discovered": [], "notes": ["base_url_unavailable"]}

    url_data = _module_data(context, "url_discovery")
    urls = url_data.get("urls") if isinstance(url_data, dict) else []
    candidates = [
        u
        for u in urls
        if isinstance(u, str) and ("/api" in u.lower() or "/graphql" in u.lower())
    ]
    candidates.extend(
        [
            urljoin(base_url.rstrip("/") + "/", "api"),
            urljoin(base_url.rstrip("/") + "/", "api/v1"),
            urljoin(base_url.rstrip("/") + "/", "graphql"),
            urljoin(base_url.rstrip("/") + "/", "swagger"),
            urljoin(base_url.rstrip("/") + "/", "openapi.json"),
        ]
    )
    candidates = list(dict.fromkeys([c for c in candidates if isinstance(c, str)]))
    candidates = candidates[:6]

    timeout_seconds = _cap_timeout(context, 2.0)
    responses: list[dict[str, Any]] = []
    for url in candidates:
        resp = _fetch_url(url, timeout_seconds=timeout_seconds, max_bytes=8000)
        responses.append(
            {
                "url": url,
                "status": resp.get("status_code"),
    ...
    exposed = [
        ...
        and item.get("status") in {200, 401, 403}
        and item.get("url")
    ]
    return {"tested": candidates, "discovered": exposed, "responses": responses}


def _module_sql_injection_test(context: dict) -> dict:
    url_data = _module_data(context, "url_discovery")
    urls = url_data.get("urls") if isinstance(url_data, dict) else []
    candidates = [u for u in urls if isinstance(u, str) and "?" in u]
    candidates = candidates[:3]
    if not candidates:
        return {"tested": [], "vulnerable": [], "notes": ["no_query_urls"]}
    ...
    for url in candidates:
        ...
        if not params:
            continue
        mutated = [(key, f"{value}'") for key, value in params]
        new_query = urlencode(mutated, doseq=True)
        attack_url = urlunparse(parsed._replace(query=new_query))
        resp = _fetch_url(attack_url, timeout_seconds=timeout_seconds, max_bytes=10000)
        snippet = str(resp.get("snippet") or "")
        snippet_lc = snippet.lower()
        matched = next((pattern for pattern in SQL_ERROR_PATTERNS if pattern in snippet_lc), None)
        entry = {
            "url": url,
            "attack_url": attack_url,
            "status": resp.get("status_code"),
            "evidence": _safe_snippet(snippet, 400),
            "matched_pattern": matched,
        }
        tested.append(entry)
        if matched:
            vulnerable.append(entry)
    return {"tested": tested, "vulnerable": vulnerable}


def _module_xss_test(context: dict) -> dict:
    url_data = _module_data(context, "url_discovery")
    urls = url_data.get("urls") if isinstance(url_data, dict) else []
    candidates = [u for u in urls if isinstance(u, str) and "?" in u]
    candidates = candidates[:3]
    ...
    for url in candidates:
        ...
        if not params:
            continue
        mutated = [(key, XSS_PAYLOAD) for key, _value in params]
        new_query = urlencode(mutated, doseq=True)
        attack_url = urlunparse(parsed._replace(query=new_query))
        resp = _fetch_url(attack_url, timeout_seconds=timeout_seconds, max_bytes=10000)
        snippet = str(resp.get("snippet") or "")
        reflected = XSS_PAYLOAD.lower() in snippet.lower()
        entry = {
            "url": url,
            "attack_url": attack_url,
            "status": resp.get("status_code"),
...
def _module_vulnerability_scan(context: dict) -> dict:
    ...
    banner = f"{server}".lower()
    candidates = [str(fw) for fw in frameworks if str(fw)]
    if server:
        candidates.append(str(server))
    ...
    cve_catalog = {
        ...
        "jenkins": ["CVE-2023-27898"],
    }

    findings: list[dict[str, Any]] = []
    for item in candidates:
        lowered = item.lower()
        for key, cves in cve_catalog.items():
            if key in lowered or key in banner:
                findings.append(
                    {
                        "product": item,
                        "cves": cves,
                        "evidence": item,
                        "note": "Version unknown; validate before remediation.",
                    }
                )
                break

    return {"potential_cves": findings}
...
def _module_vulnerability_surface(context: dict) -> dict:
    ...
    tech_data = _module_data(context, "technology_fingerprint")
    http_data = _module_data(context, "http_probe")
    sql_data = _module_data(context, "sql_injection_test")
    xss_data = _module_data(context, "xss_test")
    dir_data = _module_data(context, "directory_bruteforce")
    auth_data = _module_data(context, "auth_testing")
    session_data = _module_data(context, "session_management")
    api_data = _module_data(context, "api_endpoint_testing")
    cve_data = _module_data(context, "vulnerability_scan")
    ...
    # Using detailed dictionaries instead of just strings
    detailed_risks = []
    misconfigurations = []
    html_snippet = str(http_data.get("html_snippet") or "") if isinstance(http_data, dict) else ""
    header_snapshot = http_data.get("headers") if isinstance(http_data, dict) else {}
    raw_http_evidence = _safe_snippet(html_snippet, 500) or _safe_snippet(str(header_snapshot), 500)

    for port in open_ports:
        port = int(port)
        service = SERVICE_MAP.get(port, "unknown")
        if port in {21, 22, 25, 445, 3306, 3389}:
            detailed_risks.append({
                "title": f"Sensitive service exposed on port {port}",
                "severity": "high",
                "raw_output": f"Port {port}/tcp ({service}) is open to the public internet.",
                "mitigation": f"Block port {port} at the edge firewall or restrict access to trusted IP ranges (VPN/Bastion) immediately."
            })
        elif port in {80, 8080}:
            detailed_risks.append({
                "title": f"Unencrypted HTTP service exposed on port {port}",
                "severity": "medium",
                "raw_output": f"Port {port}/tcp is open and likely communicating in plaintext.",
                "mitigation": "Configure a redirect to HTTPS (port 443) and ensure the web server forces TLS communication."
            })
    ...
    for header_name in missing_headers:
        detailed_risks.append({
            ...
            "severity": "low",
            "raw_output": raw_http_evidence or f"HTTP Response is missing the '{header_name}' header.",
            "mitigation": f"Configure the web application or reverse proxy to inject '{header_name}' in all server responses."
        })
        misconfigurations.append(f"Header misconfiguration: {header_name}")

    if frameworks:
        detailed_risks.append({
            "title": f"Technology exposure: {', '.join(frameworks)}",
            "severity": "low",
            "raw_output": raw_http_evidence or f"Fingerprint matches found in HTML/Headers for: {', '.join(frameworks)}",
            "mitigation": "Ensure framework versions are hidden and error pages do not leak sensitive backend stacks."
        })

    vulnerable_sql = sql_data.get("vulnerable") if isinstance(sql_data, dict) else []
    for entry in vulnerable_sql or []:
        detailed_risks.append(
            {
                "title": f"Possible SQL Injection at {entry.get('url')}",
                "severity": "high",
                "raw_output": _safe_snippet(entry.get("evidence"), 500),
                "mitigation": "Use parameterized queries/ORMs and validate/encode all input fields.",
            }
        )

    vulnerable_xss = xss_data.get("vulnerable") if isinstance(xss_data, dict) else []
    for entry in vulnerable_xss or []:
        detailed_risks.append(
            {
                "title": f"Reflected XSS indicator at {entry.get('url')}",
                "severity": "high",
                "raw_output": _safe_snippet(entry.get("evidence"), 500),
                "mitigation": "Escape/encode user input and add a strict Content-Security-Policy.",
            }
        )

    sensitive_files = dir_data.get("sensitive_files") if isinstance(dir_data, dict) else []
    for entry in sensitive_files or []:
        detailed_risks.append(
            {
                "title": f"Sensitive file exposed: {entry.get('path')}",
                "severity": "high",
                "raw_output": _safe_snippet(entry.get("snippet"), 500),
                "mitigation": "Remove sensitive files from public web roots and restrict access via server rules.",
            }
        )

    auth_issues = auth_data.get("issues") if isinstance(auth_data, dict) else []
    for issue in auth_issues or []:
        if isinstance(issue, dict):
            detailed_risks.append(issue)

    session_issues = session_data.get("issues") if isinstance(session_data, dict) else []
    for issue in session_issues or []:
        if isinstance(issue, dict):
            detailed_risks.append(issue)

    api_endpoints = api_data.get("discovered") if isinstance(api_data, dict) else []
    for endpoint in api_endpoints or []:
        url = endpoint.get("url")
        status = endpoint.get("status")
        if url and status == 200 and any(token in str(url).lower() for token in ("swagger", "openapi", "graphql")):
            detailed_risks.append(
                {
                    "title": f"Public API endpoint exposed: {url}",
                    "severity": "medium",
                    "raw_output": _safe_snippet(endpoint.get("snippet"), 500),
                    "mitigation": "Restrict API documentation to authenticated users and apply access controls.",
                }
            )

    potential_cves = cve_data.get("potential_cves") if isinstance(cve_data, dict) else []
    for entry in potential_cves or []:
        product = entry.get("product")
        cves = entry.get("cves") or []
        detailed_risks.append(
            {
                "title": f"Potential CVE exposure: {product}",
                "severity": "medium",
                "raw_output": f"Potential CVEs: {', '.join(cves)} | Evidence: {entry.get('evidence')}",
                "mitigation": "Validate product versions and apply the latest security patches.",
            }
        )

    # Backward compatibility with string-based `potential_risks` list
    potential_risks = [risk["title"] for risk in detailed_risks]
...
MODULE_HANDLERS = {
    ...
    "service_enumeration": _module_service_enumeration,
    "http_probe": _module_http_probe,
    "url_discovery": _module_url_discovery,
    "osint_public_data": _module_osint_public_data,
    "search_engine_recon": _module_search_engine_recon,
    ...
    "directory_bruteforce": _module_directory_bruteforce,
    "auth_testing": _module_auth_testing,
    "session_management": _module_session_management,
    "api_endpoint_testing": _module_api_endpoint_testing,
    "sql_injection_test": _module_sql_injection_test,
    "xss_test": _module_xss_test,
    "vulnerability_scan": _module_vulnerability_scan,
...
def _run_modules(
    ...
    *,
    ...
    time_budget = int(getattr(Config, "SCAN_TIME_BUDGET_SECONDS", 0) or 0)
    ...
    context = {
        ...
        "command_timeout_seconds": int(getattr(Config, "SCAN_TOOL_TIMEOUT_SECONDS", 25)),
        "subdomain_tool_timeout_seconds": int(getattr(Config, "SCAN_SUBDOMAIN_TOOL_TIMEOUT_SECONDS", 15)),
        "url_discovery_timeout_seconds": int(getattr(Config, "SCAN_URL_DISCOVERY_TIMEOUT_SECONDS", 20)),
        "port_scan_timeout_seconds": int(getattr(Config, "SCAN_PORT_SCAN_TIMEOUT_SECONDS", 25)),
        "whois_timeout_seconds": int(getattr(Config, "SCAN_WHOIS_TIMEOUT_SECONDS", 6)),
    }
    ...
    for idx, module_name in enumerate(module_names, start=1):
        ...
        if remaining is not None and remaining <= 1:
            for leftover in module_names[idx - 1 :]:
                modules[leftover] = {"status": "skipped", "error": "time_budget_exceeded"}
                _emit_event(
                    context,
                    {
                        "type": "module_skipped",
                        ...
                        "target": normalized_target,
                        "module": leftover,
                        "stage": MODULE_STAGE.get(leftover, leftover),
                        "index": idx,
                        "total": total_modules,
    ...
    time_budget_exceeded = any(
        entry.get("status") == "skipped" and entry.get("error") == "time_budget_exceeded"
...
def generate_findings(*, scan_id: int, target: str, results: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def add_finding(
        *,
        ...
        finding = {
            ...
            "category": category,
            "status": status,
            "affected_endpoint": affected_endpoint or "N/A",
    ...
    # Vulnerability surface detailed risks (SQLi, XSS, auth/session, CVEs, etc.)
    vuln_surface = _get_module_data(results, "vulnerability_surface")
    detailed = vuln_surface.get("detailed_risks") if isinstance(vuln_surface, dict) else []
    if isinstance(detailed, list):
        for risk in detailed[:200]:
            ...
            description = str(risk.get("raw_output") or risk.get("description") or "").strip()
            if not description:
                description = title
            add_finding(
                severity=severity,
