...
def _enforce_rate_limit(app: Flask):
    ...
    if endpoint in {
        "auth.signup",
        "auth_public.signup_legacy",
        "auth.signup_page",
        "auth_public.signup_page_legacy",
        "api.auth_signup",
    } or request.path in {"/signup", "/auth/signup", "/api/auth/signup"}:
        key = f"signup:{get_client_ip()}"
        allowed, retry_after = limiter.check(
            key,
            app.config["SIGNUP_RATE_LIMIT"],
            app.config["SIGNUP_RATE_WINDOW_SEC"],
        )
        if not allowed:
            return jsonify({"error": "Too many signup attempts", "retry_after": retry_after}), 429

    scan_paths = {"/scan", "/scans", "/scans/passive", "/scans/active", "/scans/full", "/api/scans"}
    if request.path in scan_paths or request.path.startswith("/api/scans") or endpoint in {
