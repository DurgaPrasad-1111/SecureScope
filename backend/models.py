...
class User(db.Model):
    ...
    mfa = db.relationship("UserMFA", back_populates="user", uselist=False, lazy="selectin")
    ...
    def to_dict(self) -> dict:
        ...
        try:
            if self.mfa is not None:
                mfa_enabled = bool(self.mfa.enabled)
        except Exception:
            mfa_enabled = False
        return {
            "id": self.id,
...
class UserMFA(db.Model):
    ...
    enabled = db.Column(db.Boolean, nullable=False, default=False, server_default=db.text("0"))
    created_at = db.Column(db.DateTime, nullable=True, server_default=db.func.current_timestamp())
    ...
    user = db.relationship("User", back_populates="mfa", lazy="joined")
...
class Scan(db.Model):
    ...
    def to_dict(self) -> dict:
        results = self.results_json if isinstance(self.results_json, dict) else {}
        ...
        return {
            ...
            "status": self.status,
            "progress": meta.get("progress", 100 if self.status == "completed" else 0),
            "currentPhase": meta.get("current_phase", "completed" if self.status == "completed" else "queued"),
...
class Finding(db.Model):
    ...
    affected_endpoint = db.Column(db.String(512), nullable=True)
    ...
    def to_api_dict(self) -> dict:
        ...
        return {
            "id": self.id,
            "severity": (self.severity or "low").capitalize(),
            "title": self.title,
            "description": self.description or "",
            "affected_endpoint": self.affected_endpoint or "N/A",
            "raw_data": self.raw_data or {},
            "mitigation": self.mitigation or "No specific mitigation provided.",
...
class AdminAccessRequest(db.Model):
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "user": {
                "id": str(self.requester.id) if self.requester else None,
                "name": self.requester.username if self.requester else "",
                "email": self.requester.email if self.requester else "",
            },
        }
