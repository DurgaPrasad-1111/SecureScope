...
from models import AdminAccessRequest, AuditLog, Finding, Role, Scan, User, UserMFA
from services.auth_service import authenticate_user, hash_password, validate_username
from services.audit_service import log_action
from services.mfa_service import generate_totp_secret, totp_provisioning_uri, verify_totp
...
def _estimate_duration_seconds(mode: str) -> int:
    default = 90 if mode == "full" else 60
    budget = int(getattr(Config, "SCAN_TIME_BUDGET_SECONDS", 0) or 0)
    if budget <= 0:
        return default
...
def _parse_role_names(raw_roles: Any) -> list[str]:
...
def _scan_type_from_mode(mode: str) -> tuple[str, str]:
    ...
    if mode == "passive":
        return "normal_scan", "Normal Scan"
    if mode == "active":
...
def _api_user_payload(user: User) -> dict[str, Any]:
    ...
    try:
        if user.mfa is not None:
            mfa_enabled = bool(user.mfa.enabled)
    except Exception:
        mfa_enabled = False
    return {
        "id": str(user.id),
...
def _require_api_permission(permission: str) -> tuple[User, dict[str, Any]] | Any:
    auth = _require_api_auth()
    if not isinstance(auth, tuple):
        return auth
    user, payload = auth
    if not user.has_permission(permission):
        return api_error(status=403, code="FORBIDDEN", message="Insufficient permissions")
    return user, payload


def _require_api_role(role_name: str) -> tuple[User, dict[str, Any]] | Any:
    auth = _require_api_auth()
    if not isinstance(auth, tuple):
        return auth
    user, payload = auth
...
def _scans_query_for_user(user: User):
    query = Scan.query
...
def _findings_query_for_user(user: User):
    query = Finding.query.join(Scan, Finding.scan_id == Scan.id)
    if not user.has_role("admin"):
        query = query.filter(Scan.user_id == user.id)
    return query


def _require_scan_mode_permission(user: User, mode: str) -> Any | None:
    normalized = (mode or "").strip().lower()
    required_permission = "scan:passive" if normalized == "passive" else "scan:active"
    if not user.has_permission(required_permission):
        return api_error(status=403, code="FORBIDDEN", message="Insufficient permissions")
    if normalized == "full" and not user.has_role("admin"):
        return api_error(status=403, code="FORBIDDEN", message="Admin role required for full scans")
    return None


def _require_scan_access(user: User, scan: Scan) -> Any | None:
    if scan.user_id != user.id and not user.has_role("admin"):
        return api_error(status=403, code="FORBIDDEN", message="Access denied to requested scan")
    return None
...
def auth_login():
    ...
    if user is None:
        log_action(
            ...
            resource="api.auth",
            user_id=None,
            details={"target": mask_identity(email)},
        )
    ...
    if mfa_enabled:
        if not otp_code:
            ...
            return api_error(status=401, code="MFA_REQUIRED", message="Two-factor code required")
        ...
        if not verify_totp(secret=mfa_secret or "", code=otp_code):
            ...
            return api_error(status=401, code="MFA_INVALID", message="Invalid two-factor code")
...
def auth_2fa_setup():
    if not Config.ENABLE_TOTP:
        return api_error(status=403, code="MFA_DISABLED", message="Two-factor authentication is disabled")
    ...
    try:
        ...
        if mfa is None:
            mfa = UserMFA(user_id=user.id, totp_secret=generate_totp_secret(), enabled=False)
            db.session.add(mfa)
            db.session.commit()
        elif not mfa.totp_secret:
            mfa.totp_secret = generate_totp_secret()
            mfa.enabled = False
            db.session.commit()
    except SQLAlchemyError:
        db.session.rollback()
        return api_error(status=503, code="MFA_UNAVAILABLE", message="MFA storage unavailable")

    label = user.email or user.username
    otpauth_uri = totp_provisioning_uri(secret=mfa.totp_secret, label=label)
    return jsonify(
        {
            "issuer": getattr(Config, "TOTP_ISSUER", "ReconEngine"),
            "secret": mfa.totp_secret,
            "otpauthUri": otpauth_uri,
            "enabled": bool(mfa.enabled),
        }
    )


@bp.post("/auth/2fa/verify")
def auth_2fa_verify():
    if not Config.ENABLE_TOTP:
        return api_error(status=403, code="MFA_DISABLED", message="Two-factor authentication is disabled")
    ...
    payload = request.get_json(silent=True) or {}
    code = sanitize_text(payload.get("code") or payload.get("otp") or payload.get("totp") or "", max_len=12)
    if not code:
        return api_error(
            status=422,
            code="VALIDATION_ERROR",
            message="Invalid request parameters",
            details=[{"field": "code", "message": "TOTP code is required"}],
        )
    ...
    try:
        ...
        db.session.rollback()
        return api_error(status=503, code="MFA_UNAVAILABLE", message="MFA storage unavailable")
    if mfa is None or not mfa.totp_secret:
        return api_error(status=404, code="MFA_NOT_CONFIGURED", message="Two-factor authentication not configured")

    if not verify_totp(secret=mfa.totp_secret, code=code):
        log_action(
            action="auth.mfa.failed",
            resource="api.auth",
            user_id=user.id,
            details={"target": mask_identity(user.email or user.username)},
        )
        return api_error(status=401, code="MFA_INVALID", message="Invalid two-factor code")

    mfa.enabled = True
    db.session.commit()
    log_action(action="auth.mfa.enabled", resource="api.auth", user_id=user.id, details={"target": "mfa"})
...
def auth_2fa_disable():
    ...
    if mfa.enabled:
        if not code or not verify_totp(secret=mfa.totp_secret, code=code):
            return api_error(status=401, code="MFA_INVALID", message="Invalid two-factor code")

    mfa.enabled = False
    db.session.commit()
    log_action(action="auth.mfa.disabled", resource="api.auth", user_id=user.id, details={"target": "mfa"})
...
@bp.post("/auth/signup")
def auth_signup():
    if not bool(getattr(Config, "ALLOW_SIGNUP", False)):
        return api_error(status=403, code="SIGNUP_DISABLED", message="Signups are disabled")

    payload = request.get_json(silent=True) or {}
    try:
        username = validate_username(sanitize_identity(payload.get("username") or payload.get("name") or ""))
    ...
    email = sanitize_email(email_raw)
    password = str(payload.get("password") or "")
    confirm = str(payload.get("confirmPassword") or payload.get("confirm_password") or payload.get("confirm") or "")
    ...
    if password and password != confirm:
        details.append({"field": "confirmPassword", "message": "Passwords do not match"})
    if details:
        return api_error(status=422, code="VALIDATION_ERROR", message="Invalid request parameters", details=details)

    existing = User.query.filter((User.username == username) | (User.email == email)).first()
    if existing is not None:
        return api_error(status=409, code="RESOURCE_CONFLICT", message="Username or email already exists")

    try:
        seed_rbac_data()
        basic_role = Role.query.filter_by(name="basic").first()
        if basic_role is None:
            return api_error(status=500, code="RBAC_ERROR", message="Basic role missing")
        ...
        db.session.rollback()
        return api_error(status=500, code="DATABASE_ERROR", message="Failed to create user")
...
def dashboard_metrics():
    auth = _require_api_permission("scan:read")
    if not isinstance(auth, tuple):
        return auth
    user, _token_payload = auth

    scans = _scans_query_for_user(user).order_by(desc(Scan.created_at)).limit(250).all()
...
def dashboard_findings_distribution():
    ...
    scans = _scans_query_for_user(user).order_by(desc(Scan.created_at)).limit(500).all()
...
def scans_recent():
    ...
    query = _scans_query_for_user(user).order_by(desc(Scan.created_at))
    total = query.count()
                ...
                def _event_cb(event: dict[str, Any]):
                    try:
                        ...
                        if event_type == "module_started" and module_name:
                            ...
                            if "data" in event:
                                patch["modules"][module_name]["data"] = event["data"]
                        ...
                        if event_type == "module_completed" and module_name in {"vulnerability_scan", "vulnerability_surface", "sql_injection_test", "xss_test", "port_scan", "subdomain_enum"}:
                            try:
                                from models import Finding
                                db.session.refresh(scan)
                                Finding.query.filter(Finding.scan_id == scan.id).delete(synchronize_session=False)
                                current_results = scan.results_json if isinstance(scan.results_json, dict) else {}
                                payloads = generate_findings(scan_id=scan.id, target=scan.target, results=current_results)
                                ...
                                for payload in payloads:
                                    ...
                                    db.session.add(
                                        Finding(
                                            ...
                                            affected_endpoint=str(payload.get("affected_endpoint") or "N/A"),
                                ...
                                app.logger.exception("Failed to update findings during scan progress")
                                db.session.rollback()
...
def scans_create():
    ...
    if any(is_suspicious_input(item) for item in targets):
        log_action(
            action="security.suspicious_input",
            resource="api.scans.create",
            user_id=user.id,
            details={"target": "targets"},
        )

    targets = sanitize_list(targets, max_len=300)

    scan_type = sanitize_text(payload.get("scanType") or "normal_scan", max_len=32).strip()
    scan_type_map = {
        "normal_scan": "passive",
    ...
    mode = scan_type_map.get(scan_type)
    if mode is None:
        return api_error(
            status=422,
            code="VALIDATION_ERROR",
            message="Invalid request parameters",
            details=[{"field": "scanType", "message": "scanType must be one of: normal_scan, quick_scan, full_scan"}],
        )
    permission_error = _require_scan_mode_permission(user, mode)
    if permission_error is not None:
        return permission_error

    schedule = payload.get("schedule") or {"type": "immediate", "scheduledAt": None}
    schedule_type = sanitize_text(schedule.get("type") or "immediate", max_len=24).strip().lower()
    ...
    if len(normalized_targets) > MAX_TARGETS_PER_REQUEST:
        return api_error(
            status=422,
            code="VALIDATION_ERROR",
            message="Invalid request parameters",
            details=[
                {
                    "field": "targets",
                    "message": f"Too many targets (max {MAX_TARGETS_PER_REQUEST})",
                }
            ],
        )
    ...
    try:
        ...
        enforce_target_policy(target, allow_private_targets=current_app.config.get("ALLOW_PRIVATE_TARGETS", False))
    except ValueError as exc:
        return api_error(
            status=422,
            code="VALIDATION_ERROR",
            ...
            jsonify(
                {
                    ...
                    "estimatedDuration": _estimate_duration_seconds(mode),
                    "queuePosition": 1,
...
def scans_validate_targets():
    auth = _require_api_permission("scan:passive")
    if not isinstance(auth, tuple):
        return auth
    ...
    if any(is_suspicious_input(item) for item in targets):
        log_action(
            action="security.suspicious_input",
            resource="api.scans.validate",
            user_id=auth[0].id if isinstance(auth, tuple) else None,
            details={"target": "targets"},
        )
    ...
    if len(targets) > MAX_TARGETS_PER_REQUEST:
        return api_error(
            ...
            code="VALIDATION_ERROR",
            message="Invalid request parameters",
            details=[{"field": "targets", "message": f"Too many targets (max {MAX_TARGETS_PER_REQUEST})"}],
        )
    ...
    for raw in targets:
        target = sanitize_text(raw, max_len=300).strip()
        if not target:
        ...
        try:
            ...
            enforce_target_policy(normalized, allow_private_targets=current_app.config.get("ALLOW_PRIVATE_TARGETS", False))
            try:
                ipaddress.ip_address(normalized)
                valid.append({"target": normalized, "type": "ip", "resolved": True, "ipAddress": normalized})
            except ValueError:
                valid.append({"target": normalized, "type": "domain", "resolved": True, "ipAddress": None})
        except ValueError as exc:
...
def scans_get(scan_id: str):
    ...
    if scan is None:
        return api_error(status=404, code="RESOURCE_NOT_FOUND", message="Scan not found")
    access_error = _require_scan_access(user, scan)
...
def scans_cancel(scan_id: str):
    ...
    if access_error is not None:
        return access_error
    permission_error = _require_scan_mode_permission(user, scan.scan_mode)
    if permission_error is not None:
        return permission_error
...
def _assets_latest_by_target(user: User) -> dict[str, Scan]:
    scans = _scans_query_for_user(user).order_by(desc(Scan.created_at)).limit(5000).all()
    latest: dict[str, Scan] = {}
    for scan in scans:
        for asset in _asset_candidates_from_scan(scan):
            if asset not in latest:
                latest[asset] = scan
    return latest


def _asset_candidates_from_scan(scan: Scan) -> set[str]:
    results = scan.results_json if isinstance(scan.results_json, dict) else {}
    target = str(scan.target or "").strip().lower().rstrip(".")
    assets: set[str] = set()
    if target:
        assets.add(target)

    sub_data = _scan_module_data(results, "subdomain_enum")
    subdomains = sub_data.get("subdomains") if isinstance(sub_data.get("subdomains"), list) else []
    ...
    # Filter out raw IP assets for inventory views.
...
def assets_list():
    ...
    latest = _assets_latest_by_target(user)
...
def _resolve_target_by_asset_id(user: User, asset_id: str) -> str | None:
    latest = _assets_latest_by_target(user)
...
def assets_details(asset_id: str):
    ...
    target = _resolve_target_by_asset_id(user, asset_id)
    if not target:
        return api_error(status=404, code="RESOURCE_NOT_FOUND", message="Asset not found")

    scans_all = _scans_query_for_user(user).order_by(desc(Scan.created_at)).limit(500).all()
    normalized_target = str(target).strip().lower().rstrip(".")
    scans = [scan for scan in scans_all if normalized_target in _asset_candidates_from_scan(scan)]
    latest = scans[0] if scans else None
    if latest is None:
        return api_error(status=404, code="RESOURCE_NOT_FOUND", message="Asset not found")
    ...
    return jsonify(
        {
            ...
            "findings": findings,
            "metadata": {"ports": [], "technologies": [], "certificates": [], "dns": {"a": [], "aaaa": [], "mx": [], "txt": []}},
            "tags": [],
            "notes": "",
            "assignedTo": None,
        }
...
def assets_trigger_scan(asset_id: str):
    ...
    payload = request.get_json(silent=True) or {}
    scan_type = sanitize_text(payload.get("scanType") or "quick_scan", max_len=32).strip()
    ...
    if mode is None:
        return api_error(
            ...
            code="VALIDATION_ERROR",
            message="Invalid request parameters",
            details=[{"field": "scanType", "message": "scanType must be one of: quick_scan, full_scan"}],
        )
    permission_error = _require_scan_mode_permission(user, mode)
    if permission_error is not None:
        return permission_error
    try:
        ...
        return api_error(status=403, code="FORBIDDEN", message=str(exc))
...
def assets_export():
    ...
    for row in rows:
        sanitized = {
            key: _sanitize_csv_value(str(value)) if value is not None else ""
...
@bp.post("/admin-requests")
def admin_request_create():
    auth = _require_api_auth()
    if not isinstance(auth, tuple):
        return auth
    user, _payload = auth
    ...
    if user.has_role("admin"):
        return api_error(status=409, code="RESOURCE_CONFLICT", message="User is already an admin")
    ...
    try:
        req = AdminAccessRequest(user_id=user.id, reason=reason or None, status="pending")
        db.session.add(req)
        db.session.commit()

        log_action(
            action="admin.request.created",
            resource="admin_access_requests",
            user_id=user.id,
        ...
        db.session.rollback()
        return api_error(status=500, code="DATABASE_ERROR", message="Failed to submit admin request")


@bp.post("/admin/users")
def admin_create_user():
    auth = _require_api_permission("user:create")
    if not isinstance(auth, tuple):
        return auth
    user, _payload = auth
    ...
    payload = request.get_json(silent=True) or {}
    try:
        username = validate_username(sanitize_identity(payload.get("username") or ""))
    except ValueError as exc:
        return api_error(
            status=422,
            code="VALIDATION_ERROR",
    ...
    role_names = _parse_role_names(roles_raw)
    ...
    try:
        ...
        if missing:
            return api_error(
                ...
                details=[{"field": "roles", "message": f"Unknown roles: {', '.join(missing)}"}],
            )
        ...
        log_action(
            ...
            resource="users",
            user_id=user.id,
            details={"created_user_id": user_row.id, "username": user_row.username, "roles": role_names},
        )

        return jsonify({"message": "User created", "user": user_row.to_dict()}), 201
    except ValueError as exc:
        return api_error(status=400, code="VALIDATION_ERROR", message=str(exc))
    except SQLAlchemyError:
        db.session.rollback()
        return api_error(status=500, code="DATABASE_ERROR", message="Failed to create user")


@bp.get("/admin-requests/mine")
def admin_request_mine():
    auth = _require_api_auth()
    if not isinstance(auth, tuple):
        return auth
    user, _payload = auth
    ...
    requests = (
        AdminAccessRequest.query.filter_by(user_id=user.id)
...
def admin_requests_list():
    ...
    if status_filter in {"pending", "approved", "rejected"}:
        query = query.filter(AdminAccessRequest.status == status_filter)

    requests = query.order_by(AdminAccessRequest.created_at.desc()).limit(200).all()
    return jsonify({"requests": [req.to_dict() for req in requests]})


@bp.post("/admin-requests/<int:request_id>/approve")
def admin_request_approve(request_id: int):
    auth = _require_api_role("admin")
    if not isinstance(auth, tuple):
        return auth
    reviewer, _payload = auth

    req = db.session.get(AdminAccessRequest, request_id)
    if req is None:
        return api_error(status=404, code="RESOURCE_NOT_FOUND", message="Request not found")
    if req.status != "pending":
        return api_error(status=409, code="RESOURCE_CONFLICT", message="Request is already processed")

    payload = request.get_json(silent=True) or {}
    decision_reason = sanitize_reason(payload.get("decisionReason") or "")

    try:
        seed_rbac_data()
        admin_role = Role.query.filter_by(name="admin").first()
        if admin_role is None:
            return api_error(status=500, code="RBAC_ERROR", message="Admin role missing")

        target_user = db.session.get(User, req.user_id)
        if target_user is None:
            return api_error(status=404, code="RESOURCE_NOT_FOUND", message="User not found")
        ...
        req.decision_reason = decision_reason or None
        ...
        log_action(
            ...
            resource="admin_access_requests",
            user_id=reviewer.id,
            details={"request_id": req.id, "target_user_id": target_user.id},
        )

        return jsonify({"request": req.to_dict()}), 200
    except SQLAlchemyError:
        db.session.rollback()
        return api_error(status=500, code="DATABASE_ERROR", message="Failed to approve admin request")


@bp.post("/admin-requests/<int:request_id>/reject")
def admin_request_reject(request_id: int):
    ...
    try:
        ...
        db.session.rollback()
        return api_error(status=500, code="DATABASE_ERROR", message="Failed to reject admin request")
...
def findings_list():
    ...
    query = _findings_query_for_user(user)
...
def findings_update(finding_id: str):
    ...
    if not user.has_role("admin"):
        ...
        if scan is None or scan.user_id != user.id:
            return api_error(status=403, code="FORBIDDEN", message="Access denied to requested finding")
...
def notifications_list():
    auth = _require_api_permission("audit:read")
    if not isinstance(auth, tuple):
        return auth
    user, _token_payload = auth
