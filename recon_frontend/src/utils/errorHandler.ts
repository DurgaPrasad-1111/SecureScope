...
export function handleApiError(error: unknown) {
  if (!(error instanceof ApiError)) {
    ...
    if (errorStr.includes('subdomain') || errorStr.includes('Subdomain')) {
      toast.error('Subdomain scan failed, continuing with other modules');
    } else {
  ...
  switch (error.status) {
    ...
    case 503:
      ...
      if (serverMsg.toLowerCase().includes('subdomain')) {
        toast.error('Subdomain scan failed, continuing with other modules');
      } else {
