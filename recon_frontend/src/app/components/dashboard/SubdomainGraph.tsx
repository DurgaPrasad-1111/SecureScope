...
export default function SubdomainGraph({ data, loading }: Props) {
  const { nodes, edges } = useMemo(() => {
    ...
    // Increase radius based on node count to avoid overlap
    const radius = Math.max(260, subs.length * 15);
          ...
          <div>
            <div className="font-semibold">{root.id}</div>
              <div className="mt-1 text-[11px] text-slate-700">
                Ports: {(root.metadata?.openPorts || []).slice(0, 8).join(', ') || '—'}
              </div>
        ...
        data: {
          label: (
            <div>
              ...
              <div className="mt-1 text-[11px] text-slate-700">Subdomain</div>
            </div>
          ),
        },
        position: { x, y },
